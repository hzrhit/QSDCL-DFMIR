...
class REGISTRATIONModel(BaseModel):
    ...
    def calculate_DCE_loss(self, src, tgt):
        n_layers = len(self.nce_layers)
        feat_q = self.netG(tgt, src, self.nce_layers, encode_only=True, decode_only=True, flag=True) #解码器输出

        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]
            
        feat_k = self.netG(src,src, self.nce_layers, encode_only=True, decode_only=True, flag=False) #编码器输出

...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, output=[], layers=[], encode_only=False, decode_only=False, flag=False):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            decoder_layers = layers[5:]
            if flag:
               decoder_layers = [31 - x for x in decoder_layers]
            max_val = max(encoder_layers + decoder_layers)
            for layer_id, layer in enumerate(self.model):
                # print(layer_id, layer)
                encoder_feat = layer(encoder_feat)
                decoder_feat = layer(decoder_feat)
                if layer_id in encoder_layers:
                    # print("%d: adding the output of %s %d" % (layer_id, layer.__class__.__name__, feat.size(1)))
                    encoder_feats.append(encoder_feat)
                if layer_id in decoder_layers:
                    # print("%d: adding the output of %s %d" % (layer_id, layer.__class__.__name__, feat.size(1)))
                    decoder_feats.append(decoder_feat)
                else:
                ...
                if layer_id == max_val :
                    ...
                    return encoder_feats+decoder_feats  # return intermediate features alone; stop in the last layers

            return encoder_feats, decoder_feats  # return both output and intermediate features
        else:
            """Standard forward"""
